...
(function (underscore, Q, _traverse, typson) {
    ...
    var REQUIREMENT_ANNOTATIONS = ["_requiredFor", "_optionalFor", "_hiddenFor", "_visibleFor", "_includeInExamples"];
    ...
    function copyValidationKeywords(comment, to) {
        ...
        while ((annotation = annotedValidationKeywordPattern.exec(comment))) {
            ...
            if (validationKeywords.indexOf(keyword) >= 0 || context === 'metadata') {
                var value = annotationTokens.length > 1 ? annotationTokens[1] : "";
                // @default case 1: with value -> remove spaces
                if (["default", "pattern", "syntheticValue"].indexOf(keyword) !== -1 && value) {
                    // avoid spaces (todo: refactor)
                    var list = annotation[0].split(" ");
                    var str = "";
                ...
                // @default  case 2: no value -> add empty string
                if (keyword === "default" && !value || value === "") {
                    value = "";
                }
                try {
